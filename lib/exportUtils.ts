...
export async function downloadChartPdf(
  ...
  // ── Footer with Cold Lava branding ──
  doc.setFontSize(7);
  doc.setTextColor(120, 120, 140);
  doc.text(
    `Contrl · Generated ${dateStr}`,
    margin,
    pageHeight - 8,
  );
  ...
  doc.setFontSize(6);
  doc.setTextColor(100, 100, 120);
  doc.text("powered by", pageWidth - margin - 22, pageHeight - 12, { align: "right" });
  
  doc.setFontSize(9);
  doc.setTextColor(6, 182, 212);
  doc.setFont("helvetica", "bold");
  doc.text("COLD LAVA", pageWidth - margin, pageHeight - 8, { align: "right" });

  doc.save(`${sanitizeFilename(title)}.pdf`);
}

/**
 * Capture full chart section (chart + segment stats) using html2canvas.
 * Adds "Powered by Cold Lava" branding watermark.
 */
export async function captureFullChart(
  containerElement: HTMLElement,
  includeMRChart: boolean = false,
): Promise<string> {
  // Clone the element to add branding without affecting the live page
  const clone = containerElement.cloneNode(true) as HTMLElement;
  
  // If not including MR chart, remove it from clone
  if (!includeMRChart) {
    const mrChartElements = clone.querySelectorAll('[class*="Moving Range"]');
    mrChartElements.forEach(el => el.remove());
  }
  ...
  `;
  brandText.textContent = "COLD LAVA";
  
  watermark.appendChild(poweredByText);
  watermark.appendChild(brandText);
  clone.style.position = "relative";
  clone.appendChild(watermark);

  // Append clone to body temporarily (required for html2canvas)
  clone.style.position = "absolute";
  clone.style.left = "-9999px";
  clone.style.top = "-9999px";
  clone.style.width = containerElement.offsetWidth + "px";
...
/**
 * Download full chart (chart + segment stats) as PNG with branding.
 */
export async function downloadFullChartPng(
  containerElement: HTMLElement,
  title: string,
  includeMRChart: boolean = false,
): Promise<void> {
  const dataUrl = await captureFullChart(containerElement, includeMRChart);
  const link = document.createElement("a");
  link.download = `${sanitizeFilename(title)}_full.png`;
  link.href = dataUrl;
  link.click();
}

/**
 * Download full chart as branded PDF with segment statistics.
 */
export async function downloadFullChartPdf(
  containerElement: HTMLElement,
  title: string,
  includeMRChart: boolean = false,
): Promise<void> {
  const imageDataUrl = await captureFullChart(containerElement, includeMRChart);
  ...
  // ── Header with Cold Lava branding ──
  doc.setFillColor(15, 15, 15);
  ...
  // Cold Lava branding
  doc.setFontSize(9);
  doc.setTextColor(6, 182, 212); // Cyan brand color
  doc.text("Powered by Cold Lava", pageWidth - margin, 12, { align: "right" });

  // Contrl product name
  doc.setFontSize(7);
  doc.setTextColor(120, 120, 140);
  doc.text("Contrl · Statistical Process Control", pageWidth - margin, 16, { align: "right" });
  ...
  // ── Full Chart Image ──
  const cursorY = 32;
  const chartHeight = pageHeight - cursorY - 15; // Use most of the page
  ...
  doc.save(`${sanitizeFilename(title)}_full.pdf`);
}
