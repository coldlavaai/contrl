...
import { calculateEwma } from "@/lib/subgroupSpc";
...
export default function EwmaChart({
  values,
  dates,
  title = "EWMA Chart",
  unit = "",
  ...
  const result = useMemo(
    () => calculateEwma(values, dates, lambda, 3),
    [values, dates, lambda]
  );
  ...
  const { points, mean: xBar, sigma } = result;
  const dateLabels = points.map((p) => p.label);
  const ewmaValues = points.map((p) => p.ewma);
  const uclValues = points.map((p) => p.ucl);
  const lclValues = points.map((p) => p.lcl);

  // Partition by signal
  const normalPoints = points.filter((p) => p.signal === "none");
  const signalPoints = points.filter((p) => p.signal !== "none");
  ...
  const data: any[] = [
    ...
    // EWMA line
    {
      ...
      name: "EWMA",
      x: dateLabels, y: ewmaValues,
      line: { color: "#6366f1", width: 2.5 },
      hovertemplate: `<b>%{x}</b><br>EWMA: %{y:.3f}<extra></extra>`,
    },
    // UCL
    {
      type: "scatter", mode: "lines",
      name: "UCL",
      x: dateLabels, y: uclValues,
    ...
    // Normal EWMA markers
    {
      ...
      y: normalPoints.map((p) => p.ewma),
      marker: { ...commonMarker, color: "#6366f1" },
      showlegend: false,
      hoverinfo: "skip",
    },
    // Signal points (out of limits)
    {
      type: "scatter", mode: "markers",
      name: "Out of Control",
      x: signalPoints.map((p) => p.label),
      y: signalPoints.map((p) => p.ewma),
      marker: { ...commonMarker, color: "#ef4444", size: 10 },
      hovertemplate: `<b>%{x}</b><br>EWMA: %{y:.3f}<br><span style="color:#ef4444">⚠ Out of control</span><extra></extra>`,
    },
  ];
  ...
  const layout: Partial<Plotly.Layout> = {
    ...
    yaxis: {
      ...
      title: { text: `EWMA ${unit ? `(${unit})` : ""}`, font: { color: "#6b7280", size: 12 } },
      zeroline: false,
    },
    legend: {
  ...
  const signalCount = signalPoints.length;
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            EWMA Chart · λ = {lambda.toFixed(2)} · L = 3 · {points.length} points
          </p>
        </div>
        {!readOnly && (
          <button
      ...
      {!readOnly && (
        <div className="flex items-center gap-4 p-3 rounded-xl border border-white/8 bg-white/[0.02]">
          <div className="flex items-center gap-3 flex-1">
            ...
            />
            <span className="text-sm font-mono text-indigo-400 w-10 text-right">
              {lambda.toFixed(2)}
            </span>
          </div>
          <div className="flex gap-1">
            {[0.05, 0.1, 0.2, 0.3, 0.5].map((v) => (
              <button
                key={v}
                onClick={() => setLambda(v)}
                className={`px-2 py-1 rounded text-[10px] font-semibold transition-all ${
                  Math.abs(lambda - v) < 0.005
      ...
      <div className="bg-white/[0.03] border border-white/8 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2">
          ...
          <div>
            <div className="text-[10px] text-gray-600 mb-0.5">σ̂ (Estimated)</div>
            <div className="text-sm font-semibold text-gray-300">{sigma.toFixed(4)}</div>
          </div>
          <div>
            <div className="text-[10px] text-gray-600 mb-0.5">Lambda (λ)</div>
            <div className="text-sm font-semibold text-gray-300">{lambda.toFixed(2)}</div>
          </div>
          <div>
