...
import { Search, Bell, Settings, LogOut, ChevronDown, Building2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { createClient } from "@/lib/supabase";
import { getUnacknowledgedCount } from "@/lib/alerts";
...
export default function AppHeader({ sidebarCollapsed }: AppHeaderProps) {
  ...
  // Poll alert count
  React.useEffect(() => {
    const refresh = () => setAlertCount(getUnacknowledgedCount());
    refresh();
    const interval = setInterval(refresh, 5000);
    return () => clearInterval(interval);
  }, []);

  // Refresh count when alerts panel closes
  React.useEffect(() => {
    if (!showAlerts) {
      setAlertCount(getUnacknowledgedCount());
    }
  }, [showAlerts]);
        ...
        <div className="h-full bg-[#0a0a0a] border-b border-white/[0.06] flex items-center">
          ...
          <div className="flex items-center gap-1 px-4">
            ...
            >
              ...
              {alertCount > 0 && (
                <span className="absolute top-1 right-1 flex items-center justify-center min-w-[16px] h-4 px-1 rounded-full text-[10px] font-bold bg-red-500 text-white">
